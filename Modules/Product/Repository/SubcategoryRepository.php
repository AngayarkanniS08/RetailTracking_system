...
class SubcategoryRepository implements SubcategoryRepositoryInterface {
    ...
    public function findByCategory(string $categoryId): array {
        $stmt = $this->db->prepare(
            "SELECT id, name, category_id FROM subcategories WHERE category_id = ? AND user_id = current_setting('app.current_user_id')::uuid ORDER BY name"
        );
    ...
    public function findById(string $id): ?array {
        $stmt = $this->db->prepare(
            "SELECT id, name, category_id, created_at FROM subcategories WHERE id = ? AND user_id = current_setting('app.current_user_id')::uuid"
        );
        $stmt->execute([$id]);
    ...
    public function findByNameInCategory(string $categoryId, string $name): ?array {
        $stmt = $this->db->prepare(
            "SELECT id FROM subcategories WHERE category_id = ? AND LOWER(name) = LOWER(?) AND user_id = current_setting('app.current_user_id')::uuid"
        );
    ...
    public function update(string $id, string $name): array {
        $stmt = $this->db->prepare(
            "UPDATE subcategories SET name = ? WHERE id = ? AND user_id = current_setting('app.current_user_id')::uuid RETURNING id, name, category_id, created_at"
        );
    ...
    public function delete(string $id): bool {
        $stmt = $this->db->prepare("DELETE FROM subcategories WHERE id = ? AND user_id = current_setting('app.current_user_id')::uuid");
        $stmt->execute([$id]);
