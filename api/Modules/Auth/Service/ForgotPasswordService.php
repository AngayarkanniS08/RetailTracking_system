...
class ForgotPasswordService {
    ...
    public function sendResetLink(ForgotPasswordDTO $dto): void {
        ...
        try {
            ...
            $mail->SMTPAuth   = true;
            $mail->Username   = getenv('SMTP_USER') ?: 'angayarkannisurya@gmail.com';
            $mail->Password   = getenv('SMTP_PASS') ?: 'wvwz cyvg rlgx xsip';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port       = getenv('SMTP_PORT') ?: 587;
            
            $mail->setFrom(getenv('SMTP_FROM') ?: 'no-reply@yoursystem.com', getenv('SMTP_FROM_NAME') ?: 'Inventory System');
            $mail->addAddress($user['email'], $user['username']);
            $mail->Subject = 'Password Reset Request';
            $resetLink = "http://" . $_SERVER['HTTP_HOST'] . "/Modules/auth/reset-password.html?token=" . urlencode($token);
